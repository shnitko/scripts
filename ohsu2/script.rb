...
class SetShiftingVariables
  ...
  def calc_line(panel_name, date_index, phase, events_data)
    ...
    line << calc_time(events_data, panel_name, phase_index)
    ...
    line << calc_left_right(events_data)
  ...
  def calc_time(data, panel_name, phase_index)
  ...
  def calc_left_right(data)
    lefts = 0
    rights = 0
    if data['left_or_right']
      lefts = data['left_or_right'].select { |side| side == 'left' }.count
      rights = data['left_or_right'].select { |side| side == 'right' }.count
    end
    result = ''
    result << (lefts == 0 ? ',""' : ",#{lefts}")
    result << (rights == 0 ? ',""' : ",#{rights}")
    result
  end
  ...
  def output_header
    @output_header ||= (
      ...
      header << ',# Trails(12/15),# Errors,# MC_Errors,# P_Errors,# NP_Errors,Left,Right'
      header
    )
  end
  ...
  def parse(file)
    ...
    File.foreach(file).with_index do |line, index|
      ...
      collect_time(panel_name, numbers)
      collect_left_right(panel_name, numbers) if index != (total_lines-1)
    end
  end

  def collect_left_right(panel_name, data_array)
    date = data_array[0].strip
    phase_index = data_array[12].strip.to_i
    phase_index = 7 if phase_index == 8
    event = data_array[8].strip.to_i
    if event == 3
      variants = data_array[13].strip.split('__')
      left = variants.first.split('_').first
      correct = data_array[14].strip.split('_').first
      left_or_right = (left == correct ? 'right' : 'left')
    else
      left_or_right = nil
    end
    @data[panel_name][date]["phase_#{phase_index}"]['left_or_right'] = [] if @data[panel_name][date]["phase_#{phase_index}"]['left_or_right'].nil?
    @data[panel_name][date]["phase_#{phase_index}"]['left_or_right'] << left_or_right
  end
